...
CREATE TRIGGER BALALYKIN.INSERT_SEQ_TRIGER
  BEFORE INSERT ON MEDIA_PROOF
  FOR EACH ROW
  BEGIN
  IF :new.PK_ACCIDENT_ID IS NULL THEN
    SELECT ACCIDENT_SEQ.NEXTVAL
    INTO :new.FK_MEDIA_PROOF_ID
    FROM DUAL;
  END IF;
  END;
 
 DROP TRIGGER insert_SEQ_TRIGER
 
 select status from user_objects where object_name = 'INSERT_SEQ_TRIGER' and object_type ='TRIGGER'
 
 alter trigger INSERT_SEQ_TRIGER compile;
show errors;
...
--INSERTS--
INSERT INTO ACCIDENT (ACCIDENT_ID, ACCIDENT_ADDRESS) VALUES (1,'РќРћР’РћРњР«РўР�Р©Р�РќРЎРљР�Р™ РџР РћРЎРџР•РљРў 45Рљ5')
